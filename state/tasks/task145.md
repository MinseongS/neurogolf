...
## ADOPTED 20260715T083149Z
- cost: 10492 -> 9350 (points 15.8569)
- source: candidates/task145/convbias.onnx
- note: collapse (mechanisms #1+#6): deployed spent 4340B on its entry — Slice(ch0) + a seven-node 1-D occupancy-profile in-grid detector + two Where's to build the base label plane. Replaced by ONE negative-crop Conv carrying a bias: weights ch0->-10, ch1..9->-8, bias +10, pads=[0,0,-10,-10]. Off-grid cells are the all-zero one-hot so only the bias survives, giving {bg->0, wall->2, off-grid->10} in a single fp32 op. That plane IS the base label (one Cast->u8) and carries in-grid/off-grid separation per-cell, so the whole profile detector and both base Where's vanish; the two phase-walk Einsums, min/max selection, u8 chain and Pad->Equal free output are untouched. cost 10492->9350, 23->15 nodes. Differential vs deployed .onnx: 3000 guillotine (real distribution) 0 divergences + 1198 adversarial arbitrary-red-pattern 0 divergences; BOTH nets independently reproduce a NumPy reference of the rule on every instance, so the harness demonstrably discriminates. Coverage: 400/384 distinct grid shapes, 290/123 instances at full 20-extent. Kept the deployed maxa==mina tie ordering (min last -> tie paints cyan).
