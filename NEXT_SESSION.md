...
- **주의 — task110**: generator의 `colors=` 경로(STORED 원본 예제)가 큰 비주기 패턴(period>9, 행/열
  period 상이)이라 단순 주기-복원으로 STORED 통과가 안 됨. arc-gen fresh(colors=None)는 작은 주기라
  쉬움. 110은 STORED 예제까지 맞추려면 별도 처리 필요 → 우선순위 낮추거나 건너뛰기 권장.
...
## 이번 세션 검증된 사실 (참고)
- **stored 델타 ≠ Kaggle**: sub-1pt 압축 win은 Kaggle에서 ~0. 하지만 floor-break의 큰 메모리 감축은
  1:1 반영(4회 검증). → 큰 감축만 추구, 소폭 압축은 노이즈.
- **genverify: 신규 real-0 jackpot 없음.** 구조적 real-0 = 219/255(infeasible)뿐. borderline 안정화
  (118/157/209/2)는 triage 결과 전부 INFEASIBLE(모호/global) → 사실상 dead end. floor-break에 집중.
- **제출 한도 100/day** (과거 "~5/24h"는 오진단). 아껴쓸 필요 없음.
- opset>10은 기존 public base(245/400)엔 정상. opset<=10 규칙은 내가 local-score로 채택하는 net에만.
