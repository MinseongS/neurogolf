...
Pipeline (all integer-valued, exact in float32):
  ...
  output = input * (1-ring) + boxcolor_onehot * ring
"""
...
def build(task):
    ...
    # ---- B = boxcolor spatial mask = box0 frame (1x1 Conv, runtime weight) ----
    n("Conv", ["input", "bc_chan"], "B", kernel_shape=[1, 1])         # [1,1,30,30]

    # ---- h, w from B's bounding box ----
    n("ReduceMax", ["B"], "rowocc", axes=[3], keepdims=1)             # [1,1,30,1]
    n("ReduceMax", ["B"], "colocc", axes=[2], keepdims=1)             # [1,1,1,30]
    ...
    n("ReduceMax", ["rmul"], "r1", axes=[2], keepdims=1)              # [1,1,1,1]
    # r0 = min row with boxcolor : rowocc*ridx + (1-rowocc)*BIG, reduce-min
    n("Sub", ["one", "rowocc"], "nrow")
    n("Mul", ["nrow", "big"], "nrowb")
    ...
    n("Sub", ["one", "colocc"], "ncol")
    n("Mul", ["ncol", "big"], "ncolb")
    n("Add", ["cmul", "ncolb"], "cmin_src")
    n("ReduceMin", ["cmin_src"], "c0", axes=[3], keepdims=1)
    # h = r1-r0-1 ; w = c1-c0-1  (all [1,1,1,1])
    n("Sub", ["r1", "r0"], "rh0")
    n("Sub", ["rh0", "one"], "h")
    n("Sub", ["c1", "c0"], "cw0")
    n("Sub", ["cw0", "one"], "w")
    n("Mul", ["h", "w"], "hw")                                        # [1,1,1,1]

    # ---- Tpad: 5x5 top-left h x w block, runtime Conv weight ----
    # We convolve `input` directly with a [1,10,5,5] weight that is Tpad on
    # channel 0 (the black mask) and zero elsewhere, so no separate K tensor.
    ...
    n("Add", ["h", "one"], "hp1")           # h+1 (last ring row index)
    n("Add", ["w", "one"], "wp1")
    ...
    n("Less", ["ii7", "hp2"], "in_i")
    n("Less", ["jj7", "wp2"], "in_j")
    n("Cast", ["in_i"], "in_if", to=DATA_TYPE)
    n("Cast", ["in_j"], "in_jf", to=DATA_TYPE)
    ...
    # OR via add then >0
    for a, b, o in (("b_i0", "b_ih", "or1"), ("b_j0", "b_jw", "or2")):
        n("Cast", [a], a + "f", to=DATA_TYPE)
        n("Cast", [b], b + "f", to=DATA_TYPE)
        n("Add", [a + "f", b + "f"], o)
    n("Add", ["or1", "or2"], "borsum")
    n("Greater", ["borsum", "zero"], "borb")
    n("Cast", ["borb"], "borf", to=DATA_TYPE)
    n("Mul", ["within", "borf"], "F")                                # [1,1,7,7]

    # ---- ring = ConvTranspose(corner map, F), shift/crop to canvas ----
    n("ConvTranspose", ["holef", "F"], "ringbig", kernel_shape=[7, 7],
      strides=[1, 1])                                                # [1,1,36,36]
    n("Greater", ["ringbig", "zero"], "ringbb")                      # bool [1,1,36,36]
    ...
    # Where broadcasts ringb [1,1,30,30], bc_chan [1,10,1,1], input [1,10,30,30].
